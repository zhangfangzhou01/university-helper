...
public class WordTree extends HashMap<Character, WordTree> {
	...
	public List<WordInfo> matchAllWithPos(String text, int limit) {
		return matchAllWithPos(text, limit, false, false);
	}
...
//				Console.log("i: {}, j: {}, currentChar: {}", i, j, currentChar);
	...
	// 上面的函数没法体现出敏感词在原文里的位置，所以我直接把源码拿过来改了一下，加了一个返回值，返回的是敏感词在原文里的位置
	// WordInfo类包含word，start，end三个属性，分别是敏感词，敏感词在原文里的开始位置，敏感词在原文里的结束位置
	List<WordInfo> matchAllWithPos(String text, int limit, boolean isDensityMatch, boolean isGreedMatch) {
		if (null == text) {
			return null;
		}
