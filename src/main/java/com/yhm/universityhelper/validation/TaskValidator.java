...
public class TaskValidator extends com.yhm.universityhelper.validation.Validator {
    ...
    public static void validateSelect(JSONObject task) {
        ...
        Optional.ofNullable(task.getJSONObject("search"))
                .map(search -> {
                    ...
                    Optional.ofNullable(search.getStr("tags"))
                            .map(tags -> Validator.validateMatchRegex(JSON_ARRAY_REGEX, tags, "标签不合法"));
                    ...
                    Optional.ofNullable(search.getStr("taskState"))
                            .map(taskState -> Validator.validateMatchRegex("(0|1|2|3)", taskState, "任务状态不合法"));
                    ...
                    Optional.ofNullable(search.getStr("arrivalTime"))
        ...
        if (task.containsKey("sort")) {
            Optional.ofNullable(task.getStr("sortType"))
                    .map(sortType -> Validator.validateMatchRegex("(attribute|priority)", sortType, "排序类型不合法"))
                    .orElseThrow(() -> new ValidateException("必须提供排序类型attribute或priority，示例: \"sortType\": \"attribute\""));

            // 如果是priority排序，那么sort字段必须为空
            Optional.ofNullable(task.getStr("sortType"))
                    .filter("priority"::equals)
                    .map(sortType -> Validator.validateNull(task.getJSONArray("sort"), "按优先级排序时，sort字段必须为空"));

        } else {
            Optional.ofNullable(task.getStr("sortType"))
                    .map(sortType -> Validator.validateNull(sortType, "不用排序时，排序类型必须为空"));
        }
                ...
                .map(sort -> {
                    for (int i = 0; i < sort.size(); i++) {
                        ...
                        Optional.ofNullable(sortItem.getStr("field"))
                                .map(field -> Validator.validateMatchRegex("(taskId|userId|releaseTime|taskState|arrivalTime|arrivalLocation|targetLocation|transactionAmount|isHunter|expectedPeriod|distance|phoneNumForNow|title|requireDescription|takeoutId|maxNumOfPeopleTake|score)", field, "排序字段不合法"));
                    }
                    return sort;
                });
    }
    ...
    public static void validateDeleteTaskByTaker(Long taskId, Long userId) {
        Optional.ofNullable(taskId)
                .map(Object::toString)
                .map(id -> Validator.validateNumber(id, "任务ID不合法"))
                .map(id -> TaskValidator.validateBetween("任务ID", id, 1, Long.MAX_VALUE))
        ...
        Validator.validateTrue(BeanUtils.getBean(UsertaketaskMapper.class).exists(new LambdaQueryWrapper<Usertaketask>().eq(Usertaketask::getTaskId, taskId).eq(Usertaketask::getUserId, userId)), "用户并未接取当前任务，错误的删除");
    }

    public static void validateTake(Long taskId, Long userId) {
        Optional.ofNullable(taskId)
                .map(Object::toString)
                .map(id -> Validator.validateNumber(id, "任务ID不合法"))
                .map(id -> TaskValidator.validateBetween("任务ID", id, 1, Long.MAX_VALUE))
        ...
        Validator.validateTrue(BeanUtils.getBean(UsertaketaskMapper.class).exists(new LambdaQueryWrapper<Usertaketask>().eq(Usertaketask::getTaskId, taskId).eq(Usertaketask::getUserId, userId)), "用户已接取当前任务，错误的接取");
    }
}